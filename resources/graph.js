...
// ─── Fluent webview icons ─────────────────────────────────────────────────
// Manifest and tree-view surfaces use VS Code ThemeIcons; custom webview chrome
// uses the shared Windows 11 Fluent-style SVG helper loaded before this script.
function fluentIcon(name) {
  return (globalThis.VSGIT_FLUENT_ICONS && globalThis.VSGIT_FLUENT_ICONS.icon(name)) || '';
}

const FLUENT_ICONS = {
  branch: fluentIcon('branch'),
  tag: fluentIcon('tag'),
  stash: fluentIcon('stash'),
  commit: fluentIcon('commit'),
  download: fluentIcon('fetch'),
  refresh: fluentIcon('refresh'),
  search: fluentIcon('search'),
  check: fluentIcon('check'),
  info: fluentIcon('info'),
  close: fluentIcon('close'),
  fileList: fluentIcon('fileList'),
  fileTree: fluentIcon('fileTree'),
  chevron: fluentIcon('chevron'),
  pull: fluentIcon('pull'),
  push: fluentIcon('push'),
  merge: fluentIcon('merge'),
  columns: fluentIcon('columns'),
  tracking: fluentIcon('tracking'),
  trace: fluentIcon('trace'),
};
// Toolbar data-icon aliases.
FLUENT_ICONS.fetch = FLUENT_ICONS.download;
FLUENT_ICONS.find = FLUENT_ICONS.search;
function paintToolbarIcons() {
  document.querySelectorAll('.tb-ico[data-icon]').forEach((el) => {
    el.innerHTML = FLUENT_ICONS[el.dataset.icon] || '';
  });
}
  ...
  render() {
    ...
    for (let i = 0; i < this.options.length; i++) {
      ...
      html += '<div class="dropdownOption' + (this.optionsSelected[i] ? ' ' + CLASS_SELECTED : '') + '" data-id="' + i + '" title="' + escapedName + '">' +
        (this.multipleAllowed && this.optionsSelected[i] ? '<div class="dropdownOptionMultiSelected">' + FLUENT_ICONS.check + '</div>' : '') +
        escapedName +
        (this.showInfo ? '<div class="dropdownOptionInfo" title="' + esc(this.options[i].value) + '">' + FLUENT_ICONS.info + '</div>' : '') +
        '</div>';
    }
...
function refGlyph(type) {
  if (type === 'tag') return FLUENT_ICONS.tag;
  if (type === 'stash') return FLUENT_ICONS.stash;
  return FLUENT_ICONS.branch; // head / localBranch / remoteBranch
}
...
function openExpandedRow(commit, tr) {
  ...
  cdv.innerHTML =
    '<div id="cdvHeader">' +
      ...
      '<div id="cdvViewTree" class="cdvHeaderBtn cdvViewBtn" title="Tree View">' + FLUENT_ICONS.fileTree + '</div>' +
      '<div id="cdvViewList" class="cdvHeaderBtn cdvViewBtn" title="List View">' + FLUENT_ICONS.fileList + '</div>' +
      '<div id="cdvClose" class="cdvHeaderBtn" title="Close">' + FLUENT_ICONS.close + '</div>' +
    '</div>' +
    '<div id="cdvBody">' +
...
function renderTreeLevel(node, parent, onOpen, depth) {
  Array.from(node.dirs.keys()).sort((a, b) => a.localeCompare(b)).forEach((name) => {
    ...
    chev.innerHTML = FLUENT_ICONS.chevron;
